...
df = pd.read_csv('greedy-bottom-up.csv', sep="\t")
plt.rcParams.update({'font.size': 16})
plt.figure(figsize=(9, 5), dpi=300)

ratio = df["# blocking edges"] / df["# edges"] * 100
pos = df["bottom-up - top-down"] >= 0
neg = df["bottom-up - top-down"] < 0
...
plt.yticks(range(-100,101,50), labels=[100,50,0,50,100])
plt.xlabel('Ratio of Blocking Edges (%)')
plt.ylabel('Ratio of Workflows (%)')
plt.legend()
plt.savefig('greedy-bottom-up-to-down-diff.pdf', format='pdf')
plt.show()
